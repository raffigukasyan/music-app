...
import {SidebarContext} from "@/widgets/Sidebar/ui/Sidebar.tsx";
...
export const NavigationItem:FC<NavigationItemProps> = ({route, isActive}): JSX.Element => {
    ...
    const {variantsElement, variantsItem} = useContext<Variants | undefined>(SidebarContext)
    ...
    return (
        <motion.div initial={{width: '3.5ren'}}  variants={variantsItem}
            ...
            {<Icon className={`w-6 h-6 ${isActive ? 'text-myGreen' : 'text-myWhite '}`}
                   type={route.icon}/>}
            <motion.span initial={{opacity: 0, display: 'none'}}  variants={variantsElement}  transition={{ duration: 0.4, ease: "easeInOut" }}  className={isActive ? 'text-myGreen' : 'text-myWhite'}>{route.title}</motion.span>
        </motion.div>
    )
}